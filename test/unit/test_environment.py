...
from sagemaker_containers.beta.framework import env, params
import test
...
@patch('sagemaker_containers.beta.framework.env._read_json', lambda x: {
    'a': 1})
@patch('json.loads')
def test_read_exception(loads):
    loads.side_effect = ValueError('Unable to read.')

    assert env.read_hyperparameters() == {
        'a': 1}
...
def create_training_env():
    with patch('sagemaker_containers.beta.framework.env.read_resource_config', lambda: RESOURCE_CONFIG), \
         patch('sagemaker_containers.beta.framework.env.read_input_data_config', lambda: INPUT_DATA_CONFIG), \
         patch('sagemaker_containers.beta.framework.env.read_hyperparameters', lambda: ALL_HYPERPARAMETERS), \
         patch('sagemaker_containers.beta.framework.env.num_cpus', lambda: 8), \
         patch('sagemaker_containers.beta.framework.env.num_gpus', lambda: 4):
        session_mock = Mock()
        session_mock.region_name = 'us-west-2'
        old_environ = os.environ.copy()
        os.environ[params.TRAINING_JOB_ENV] = 'training-job-42'

        yield sagemaker_containers.beta.framework.training_env()
...
def create_serving_env():
    with patch('sagemaker_containers.beta.framework.env.num_cpus', lambda: 8), \
         patch('sagemaker_containers.beta.framework.env.num_gpus', lambda: 4):
        old_environ = os.environ.copy()
...
def test_env_dictionary():
    ...
    assert environment['module_name'] == 'my_app'
    assert environment['log_level'] == logging.INFO
